...
const LEGACY_SITE_COOKIE = "ventiq_site_access";
...
function getAppAccessSecret() {
  return (
    process.env.VENTIQ_APP_ACCESS_SECRET ||
...
function hasLegacyPageAccess(request: NextRequest) {
  const expectedToken = process.env.SITE_LOCK_TOKEN || "";
  const cookieToken = request.cookies.get(LEGACY_SITE_COOKIE)?.value || "";
...
export async function proxy(request: NextRequest) {
  ...
  // Temporary compatibility only for old browser QA sessions.
  // It may open the page shell, but PrivateRouteGate still requires a real
  // Supabase VENTIQ session before private content is rendered.
  // Legacy site-lock access is deliberately NOT accepted for private APIs.
  if (!pathname.startsWith("/api") && hasLegacyPageAccess(request)) {
    return privateResponse(NextResponse.next());
  }
