...
async function main() {
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      console.log("No A7-4D temporary account state exists. Nothing to clean.");
      return;
    }
    throw error;
  }
  ...
  if (!userId) throw new Error("A7-4D state does not contain user_id.");

  console.log("");
  console.log("VENTIQ A7-4D Temporary LP Cleanup");
  console.log("=================================");
  ...
  if (authError) throw new Error(`Auth user cleanup failed: ${authError.message}`);
  ...
  console.log("PASS - local A7-4D state file removed");
  console.log("");
  console.log("PASS - A7-4D TEMPORARY LP ACCOUNT CLEANED");
}
