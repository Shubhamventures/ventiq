...
const STATE_FILE = path.resolve(".a7-4d-temp-investor.json");
...
async function main() {
  console.log("");
  console.log("VENTIQ A7-4D Temporary LP Browser Account");
  console.log("==========================================");
  ...
  try {
    ...
    fail(
      "A temporary A7-4D account state file already exists.",
      `Run the A7-4D cleanup script first or inspect: ${STATE_FILE}`
    );
  } catch (error) {
  ...
  if (investorError || !investor) {
    fail("canonical A3TEST001 investor record was not found", investorError?.message);
  }
  pass("canonical A3TEST001 investor master found");

  const rl = createInterface({ input, output });
  const password = await rl.question(
    "Choose a temporary password for the A7-4D LP login (minimum 10 characters): "
  );
  rl.close();
  ...
  const email = `a7.lp.documents.${Date.now()}@ventiq.test`;
  ...
  const { data: authData, error: authError } =
    await admin.auth.admin.createUser({
      ...
      email_confirm: true,
      user_metadata: {
        full_name: "A7 Documents Browser LP - A3TEST001",
        ventiq_test_only: true,
        ventiq_test_stage: "A7-4D",
      },
    });
  ...
  try {
    ...
    if (profileError) fail("investor profile provisioning", profileError.message);
    ...
    if (membershipError) fail("organisation membership provisioning", membershipError.message);
    ...
    if (fundAccessError) fail("read-only fund access provisioning", fundAccessError.message);
    ...
    if (entitlementError) fail("investor entitlement provisioning", entitlementError.message);
    pass("A3TEST001 document entitlement ready");
    ...
    console.log("");
    console.log("==========================================");
    console.log("PASS - A7-4D TEMPORARY LP ACCOUNT READY");
    console.log("");
    console.log(`LOGIN EMAIL: ${email}`);
    console.log("LOGIN PASSWORD: use the password you just entered locally");
    console.log("");
    console.log("Open: http://localhost:3000/auth/login");
    console.log("Then open: http://localhost:3000/investor-portal");
    console.log("Click: View My Documents");
    console.log(`Expected investor code: ${INVESTOR_CODE}`);
    console.log("Expected document access: 1 canonical SOA, View + Download");
    console.log("");
