...
async function main() {
  console.log("");
  console.log("VENTIQ A7-1C Temporary LP Browser Account");
  console.log("==========================================");
  ...
  try {
    ...
    fail(
      "A temporary A7-1C account state file already exists.",
      `Run the cleanup script first or inspect: ${STATE_FILE}`
    );
  } catch (error) {
  ...
  const password = await rl.question(
    "Choose a temporary password for the A7 LP login (minimum 10 characters): "
  );
  rl.close();
  ...
  const email = `a7.lp.a3test001.${Date.now()}@ventiq.test`;
  ...
  const { data: authData, error: authError } =
    await admin.auth.admin.createUser({
      ...
      email_confirm: true,
      user_metadata: {
        full_name: "A7 Test LP - A3TEST001",
        ventiq_test_only: true,
        ventiq_test_stage: "A7-1C",
      },
    });
  ...
  try {
    ...
    pass("Active investor profile created with investor_id A3TEST001");
    ...
    pass("read-only fund access created");
    ...
    pass("A3TEST001 investor entitlement ready");
    ...
    console.log("");
    console.log("==========================================");
    console.log("PASS - A7-1C TEMPORARY LP ACCOUNT READY");
    console.log("");
    console.log(`LOGIN EMAIL: ${email}`);
    console.log("LOGIN PASSWORD: use the password you just entered locally");
    console.log("");
    console.log("Open: http://localhost:3000/auth/login");
    console.log("Expected role after login: Investor");
    console.log(`Expected fund: ${FUND_NAME}`);
    console.log(`Expected investor code: ${INVESTOR_CODE}`);
    console.log("");
    console.log("Do NOT run cleanup until the browser runtime test is finished.");
    console.log(`Cleanup state saved locally at: ${STATE_FILE}`);
  } catch (error) {
    console.log("");
    console.log("Provisioning failed; cleaning the temporary auth user...");
    await admin
      .from("ventiq_user_investor_access")
      .delete()
      .eq("user_id", userId);
