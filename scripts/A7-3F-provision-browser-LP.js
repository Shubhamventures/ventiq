...
const STATE_FILE = path.resolve(".a7-3f-temp-investor.json");
...
async function main() {
  console.log("");
  console.log("VENTIQ A7-3F Temporary LP Browser Account");
  console.log("==========================================");
  ...
  try {
    ...
    fail(
      "A temporary A7-3F account state file already exists.",
      `Run the A7-3F cleanup script first or inspect: ${STATE_FILE}`
    );
  } catch (error) {
  ...
  const password = await rl.question(
    "Choose a temporary password for the A7-3F LP login (minimum 10 characters): "
  );
  rl.close();
  ...
  const email = `a7.lp.cashflow.${Date.now()}@ventiq.test`;
  ...
  const { data: authData, error: authError } =
    await admin.auth.admin.createUser({
      ...
      email_confirm: true,
      user_metadata: {
        full_name: "A7 Cashflow Browser LP - A3TEST001",
        ventiq_test_only: true,
        ventiq_test_stage: "A7-3F",
      },
    });
  ...
  try {
    ...
    if (membershipWrite.error) {
      fail("organisation membership provisioning", membershipWrite.error.message);
    }
    ...
    if (entitlementWrite.error) {
      fail("investor entitlement provisioning", entitlementWrite.error.message);
    }

    pass("A3TEST001 cashflow/financial/document entitlement ready");
    ...
    console.log("");
    console.log("==========================================");
    console.log("PASS - A7-3F TEMPORARY LP ACCOUNT READY");
    console.log("");
    ...
    console.log("Expected cashflow access: Enabled");
    console.log("");
